...
pvalue_model <- as.data.frame(matrix(NA, nrow = dim(pheno)[1], ncol = 4))
rownames(pvalue_model) <- rownames(pheno)
colnames(pvalue_model) <- c("pvalue", "beta", "sd_beta", "N")
for (i in 1:nrow(pheno)) {
  ...
  model_o <- lm(model_form, data = test_protein, na.action = na.exclude)
  model_i <- summary(model_o)
  pvalue_model[i, 4] <- nrow(model.frame(model_o))
